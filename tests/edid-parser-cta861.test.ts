...
function getDataBlocks(parsed: ParsedEdid, index = 0): DataBlock[] {
...
describe('CTA-861 Extension Parsing', () => {
  describe('Extension Block Header', () => {
    it('should parse CTA extension header fields', () => {
      ...
      expect(extension?.underscan).toBe(true);
      expect(extension?.basicAudio).toBe(true);
      expect(extension?.ycbcr444).toBe(true);
      expect(extension?.ycbcr422).toBe(true);
    });
  });
  ...
  describe('Audio Data Block Parsing', () => {
    it('should parse audio data block and LPCM descriptor', () => {
      ...
      expect(lpcmDescriptor?.format).toBe(1);
      expect(lpcmDescriptor?.maxChannels).toBe(2);
      expect(lpcmDescriptor?.sampleRates).toBeGreaterThan(0);
    });
  });
